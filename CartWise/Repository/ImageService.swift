...
class ImageService: ImageServiceProtocol {
    ...
    // Fetches image URL for a product from MealMe API
    func fetchImageURL(for productName: String, brand: String? = nil, category: String? = nil) async throws -> String? {
        do {
            ...
            print("ImageService: Searching for image with query (MealMe): '\(searchQuery)'")
            let products = try await networkService.searchProductsOnMealMe(by: searchQuery)
